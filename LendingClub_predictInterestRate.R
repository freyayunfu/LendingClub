...
loan.regression <- glm(loan$int_rate ~ ., data = loan, family = binomial)
str(loan)
na.omit(loan)
loan.noNA <- loan[complete.cases(loan),]
set.seed(123)
boruta.train <- Boruta(loan$int_rate~., data = loan.noNA, doTrace = 2)
boruta.train <- Boruta(LoanStatus~.-LoanID, data = traindata, doTrace = 2)

data <- read.csv(file="~/Documents/AI_bittiger/lending club/loan.csv", header=TRUE, sep=",",stringsAsFactors= FALSE)
loan <- data
...
library(missForest)
loan.mis <- prodNA(loan, noNA = 0.05) 
library(reshape2)
library(ggplot2)
library(dplyr)
...
ggplot_missing(loan.mis)
install.packages("Amelia")
library(Amelia)
missmap(loan.mis)
amelia_loan <- amelia(bank.mis, m=3, parallel = "multicore",noms=c('id','member_id','term','grade','sub_grade','emp_title',
...
t.test(int_rate ~ home_ownership, data=loan)

short_term <- subset(loan,term=="36 months")
long_term <- subset(loan,term=="60 months")
diff.mean <- mean(short_term$int_rate) - mean(long_term$int_rate)
stderr <- sqrt(var(short_term$int_rate)/dim(short_term)[1]+var(long_term$int_rate)/dim(long_term)[1])
t.score <- diff.mean / stderr



round(with(loan,table(term,grade)) / as.numeric(table(loan$term)),2)
with(loan,chisq.test(grade,term))
with(loan,chisq.test(short_term$int_rate,pymnt_plan))
sort(colnames(loan))


loan2 <- c(loan$mths_since_last_delinq, loan$mths_since_last_major_derog, loan$mths_since_last_record,
           loan$mths_since_rcnt_il,loan$tot_cur_bal,loan$total_pymnt,loan$total_pymnt_inv,loan$total_rec_late_fee,
           loan$tot_coll_amt,loan$total_cu_tl,loan$total_rec_int)

loan3 <- c(loan$mths_since_last_delinq, loan$mths_since_last_major_derog, loan$mths_since_last_record,
           loan$mths_since_rcnt_il)
loan2.mis <- prodNA(loan3, noNA = 0.1)

pca1 = prcomp(loan2, scale. = TRUE)
...
train <- loan[train.ind,]
test <- loan[-train.ind,]
train.sub <- train[,c('int_rate','home_ownership','annual_inc','term','loan_amnt','total_acc',
                      'tot_cur_bal','open_acc','total_pymnt','total_pymnt_inv','total_rec_prncp',
                      'mths_since_last_delinq','mths_since_last_major_derog','total_rec_int','total_rec_late_fee')]
num.NA <- sort(sapply(train.sub,function(x){sum(is.na(x))}))
train.sub$tot_cur_bal[which(is.na(train.sub$tot_cur_bal))] <- median(train.sub$tot_cur_bal,na.rm=T)
train.sub$annual_inc[which(is.na(train.sub$annual_inc))] <- median(train.sub$annual_inc,na.rm=T)
train.sub$loan_amnt[which(is.na(train.sub$loan_amnt))] <- median(train.sub$loan_amnt,na.rm=T)
train.sub$total_acc[which(is.na(train.sub$total_acc))] <- median(train.sub$total_acc,na.rm=T)
train.sub$open_acc[which(is.na(train.sub$open_acc))] <- median(train.sub$open_acc,na.rm=T)
train.sub$total_pymnt[which(is.na(train.sub$total_pymnt))] <- median(train.sub$total_pymnt,na.rm=T)
train.sub$total_pymnt_inv[which(is.na(train.sub$total_pymnt_inv))] <- median(train.sub$total_pymnt_inv,na.rm=T)
train.sub$total_rec_prncp[which(is.na(train.sub$total_rec_prncp))] <- median(train.sub$total_rec_prncp,na.rm=T)
train.sub$mths_since_last_delinq[which(is.na(train.sub$mths_since_last_delinq))] <- 
  median(train.sub$mths_since_last_delinq,na.rm=T)
train.sub$mths_since_last_major_derog[which(is.na(train.sub$mths_since_last_major_derog))] <- median(train.sub$mths_since_last_major_derog,na.rm=T)
train.sub$total_rec_int[which(is.na(train.sub$total_rec_int))] <- median(train.sub$total_rec_int,na.rm=T)
train.sub$total_rec_late_fee[which(is.na(train.sub$total_rec_late_fee))] <- median(train.sub$total_rec_late_fee,na.rm=T)

mod2 <- lm(int_rate~.,data=train.sub)
summary(mod2)

train.con.sub <- train.sub[,c('int_rate','annual_inc','loan_amnt','total_acc',
                             'tot_cur_bal','open_acc','total_pymnt','total_pymnt_inv',
                             'total_rec_prncp','mths_since_last_delinq',
                             'mths_since_last_major_derog','total_rec_int','total_rec_late_fee')]
pca1 = prcomp(train.con.sub, scale. = TRUE)
pca1$sdev
head(pca1$rotation)
head(pca1$x)


train.cat.sub <- train[,c('int_rate','home_ownership','verification_status','term','grade','pymnt_plan',
                      'delinq_2yrs','earliest_cr_line')]
mod3 <- lm(int_rate~.,data=train.cat.sub)
summary(mod3)
